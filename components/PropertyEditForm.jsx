...
const PropertyEditForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchPropertyData = async () => {
      try {
        ...
        // Check rates for null, if so then make empty string (when it's empty, it's set to null in DB so it throws error)
  ...
  const handleChange = (e) => {
    ...
    // Check if nested fields like location.street to work properly with the state object fields
    if (name.includes(".")) {
      const [outerKey, innerKey] = name.split("."); // rates.nightly -> rates nightly
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // FormData funguje pouze pro získání dat z formulářů a jejich uložení do instance třídy FormData. Tato třída poskytuje metody pro práci s těmito daty, jako je například get(), getAll(), append() atd. Tato funkcionalita usnadňuje manipulaci s daty z formuláře před jejich odesláním na server pomocí HTTP požadavku.
